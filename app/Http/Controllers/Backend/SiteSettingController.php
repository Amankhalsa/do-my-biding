 ...
 $request->validate([
            ...
            'banner.required' => 'Please select banner photo',
                ...
                if ($request->file('banner')) {
                        ...
                        $storebanner->created_at = Carbon::now();
                        $storebanner->save();
                            $notification = array(
                                'message' => 'Site Banner published successfully',
                                'alert-type' => 'success'
                            );
...
//====================  Update front end banner image ==================== 
        public function update_front_banners(Request $request, $id){
            ...
            $banner_old_image = $request->banner_old_image;
                if ($request->file('banner')) {
                        $image = $request->file('banner');
                        $name_gen =hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
                        Image::make($image)->fit(1600,676)->save(public_path('upload/banner/'.$name_gen));
                  ...
                  if (file_exists($banner_old_image)) 
                { 
                    unlink($banner_old_image); 
                }
              
                        $bannerdata = FrontBanner::find($id);
                        $bannerdata->banner =$save_url;
                        $bannerdata->created_at = Carbon::now();
                        $bannerdata->save();
                            $notification = array(
